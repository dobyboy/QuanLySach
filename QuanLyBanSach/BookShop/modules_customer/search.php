...
if (isset($_POST['oke'])) {
    if (!empty($_POST['search'])) {
        ...
        var_dump($search);
        header("Location: ../main_customer/allitem.php?search&q=$search");
...
?>
            <div class="row">
                <div class="col-lg-2">
                    <?php
                    $query= mysqli_query($conn,"Select * from khuyen_mai WHERE id_sach='" . $row_search['id_sach'] . "'");
                ...
                <div class="col-lg-7">
                    ...
                    if ($num > 0){
                        ?>
                        <h3><a id="newcost" style="font-size: 13px; font-weight: bold;"><?php echo number_format( $row_search['gia'] - ($rowkm['gia_tri']*$row_search['gia'])/100)?> đ
                                &nbsp;</a>
                        </h3>
                        <strike><?php echo number_format($row_search['gia']) ?></strike>
                    <?php }else{ ?>
                        <h3><a id="newcost" style="font-size: 13px; font-weight: bold;"><?php echo number_format( $row_search['gia'])?> đ
                            &nbsp;</a>
                        </h3><?php } ?>
