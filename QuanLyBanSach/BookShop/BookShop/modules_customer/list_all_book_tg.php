...
<?php
$sql_tg = mysqli_query($conn, "select * from tac_gia   WHERE id_tac_gia='$_GET[id_tg]'");

while ($dong_tg = mysqli_fetch_array($sql_tg)) {

    echo '<div id="title">
                <h3 style="font-weight: bold; color: #005cbf;"><label style="color: #1a9024;font-size: 20px;">Bộ lọc theo tên tác giả &gt;&gt;</label>' . $dong_tg['ten_tac_gia'] . ' </h3><hr style="border: 1.5px solid #005cbf ;"></div>';
    ...
    $sql_loaisp = " SELECT *
				...
				WHERE s.id_tac_gia='" . $dong_tg['id_tac_gia'] . "'";
        ...
        while ($dong = mysqli_fetch_array($row)) {
            ...
            <div class="row">
                ...
                <div class="col-lg-7">
                    ...
                    if ($num > 0){
                        ?>
                        <h3><a id="newcost" style="font-size: 13px; font-weight: bold;"><?php echo number_format( $dong['gia'] - ($rowkm['gia_tri']*$dong['gia'])/100)?> đ
                                &nbsp;</a>
                        </h3>
                        <strike><?php echo number_format($dong['gia']) ?></strike>
                    <?php }else{ ?>
                ...
                <div class="col-lg-3">
                    ...
                    <button type="submit" class="btn btn-info btn-lg" name="muahang"
                            onclick="window.location.href='allitem.php?xemchitetsp&id_tg=<?php echo $dong_tg['id_tac_gia'] ?>&idsp=<?php echo $dong['id_sach'] ?>&show=1';">
